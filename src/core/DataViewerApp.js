...
export class DataViewerApp {
  ...
  initializeFormatHandlers() {
    // Register only Parquet format handler
  ...
  async handleFileInput(input, options = {}) {
    try {
      ...
      if (format !== 'parquet') {
        throw new Error('Only Parquet files are supported. Please select a .parquet file.');
      }
      ...
      this.emit('success', `Successfully loaded ${this.currentData.length} rows from Parquet file`);
